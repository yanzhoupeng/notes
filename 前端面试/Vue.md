...
`Vue.set()` 是 Vue.js 提供的一个全局 API，用于向响应式对象添加新属性并确保这个新属性也是响应式的。

**触发视图更新**：确保添加属性后能触发视图的重新渲染

**核心实现机制**
1. **检查目标类型**：首先判断目标是数组还是普通对象
2. **数组处理**：如果是数组，使用 `splice` 方法添加/修改元素
3. **对象处理**：如果是对象，检查属性是否存在
    - 如果属性已存在且值相同，直接返回
    - 如果属性不存在或值不同，添加/修改属性
4. **触发依赖通知**：通过 `dep.notify()` 通知所有依赖进行更新
...
#### v-for 的 key
`v-for` 中的 `key` 是 Vue.js 用于跟踪每个节点身份的特殊属性，主要作用有：

1. **虚拟 DOM 高效更新**：帮助 Vue 识别哪些元素被添加、删除或重新排序
2. **组件状态维护**：确保组件实例在重新渲染时能够正确复用
3. **性能优化**：减少不必要的 DOM 操作，提高渲染效率

如果使用索引值作为v-for的key，可能会导致以下问题：
1. **错误的状态复用**：
    - 当列表顺序改变时，Vue 会错误地复用组件/DOM 元素
    - 可能导致表单输入等有状态组件显示错误的值
2. **性能下降**：
    - 索引变化会导致 Vue 认为所有元素都需要更新
    - 无法利用虚拟 DOM 的差异比较优化
3. **数据不一致**：
    - 在添加/删除列表项时，可能导致渲染结果与数据不同步
    - 特别是对于动态列表操作(排序、过滤等)
#### 虚拟 DOM 和 diff 算法
